...
X_test_20p, y_test_20p, _ = preprocess_data("C:\\Users\\HP\\OneDrive\\Desktop\\BTP_5thsem\\BTP\\KDDTest+.txt", encoder=encoder_20p)
# Combine train and test datasets for KDDTrain+_20Percent
X_test_data= X_test_20p[:, :158] 
...
for name, clf in classifiers.items():
    ...
    print(classification_report(y_test_20p, y_test_pred, zero_division=0))
...
def plot_comparison(metric_name, values_full, values_20p, color1, color2):
    ...
    plt.title(f'Comparison of {metric_name} between KDDTrain+ and KDDTrain+_20Percent with Test+ data')
